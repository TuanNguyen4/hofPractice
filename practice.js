...
var onlyOneFruit = function(fruits, targetFruit) {
  var basket = _.filter(fruits, function(fruit, index, collection) {
...
var cookiesOnly = function(desserts) {
  return _.filter(desserts, function (desert, index, collection) {
    if (desert.type === 'cookie') {
      return desert;
    }
  });
};
...
var dessertCategories = function(desserts) {
  ...
  return _.reduce(desserts, function(count, dessert) {
    if (count.hasOwnProperty(dessert.type) === false) {
...
var movieNight = function(movies, timeLimit) {
  return _.reduce(movies, function(haveTime, movie) {
...
var applyCoupon = function(groceries, coupon) {
  return _.map(groceries, function(item, index, collection) {
    var itemPrice = Number(item.price.slice(1)); //'$12.1' -> 12.1
    var salePrice = itemPrice * (1 - coupon); // 12.1 * 0.8 = 9.68
    item.salePrice = '$' + salePrice.toFixed(2); // '$9.68'
    return item;
  });
};
